import { PID, Recognizer, Token, TokenFactory, UNKNOWN } from '../../src/tokenizer';
...
function* convertQuantityOfThem(tokens: Token[], factory: TokenFactory<Token>) {
    ...
    while (!sequence.atEOF()) {
        ...
        if (sequence.atEOF() || token1.type !== QUANTITY) {
            ...
            if (token2.type !== UNKNOWN || !token2.text.startsWith('of them'))
            {
                yield token1;
                yield token2;
            }
