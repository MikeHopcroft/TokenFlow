import { NumberMatch, NumberParser } from './number-parser';
...
export class EnglishNumberParser implements NumberParser {
    ...
    // [1..9]
    private QD: Map<Hash, Period>;

    // [1..19]
    private QU: Map<Hash, Period>;

    // [a, 1..9, 11..19]
    private QUX10: Map<Hash, Period>;

    // [20, 30, 40, 50, 60, 70, 80, 90]
    private QT: Map<Hash, Period>;

    private magnitudes: Map<Hash, Period>;
    ...
    constructor(stemAndHash: StemAndHash) {
        ...
        this.M2 = this.stemAndHashInternal('hundred');
        this.Z = this.stemAndHashInternal('zero');
    ...
    private createMap(items: Array<[string, number]>): Map<Hash, Period> {
        const m = new Map<Hash, Period>();
        for (const [text, value] of items) {
            const hash = this.stemAndHashInternal(text);
            m.set(hash, { value, magnitude: 0, length: 1 });
        }
        return m;
    }

    private createMagnitudeMap(items: Array<[string, number]>): Map<Hash, Period> {
        const m = new Map<Hash, Period>();
